...
const countries = [
 ...
 // console.log(countries);
  ...
  const newObjetos = (array) => {
    countries.forEach((countrie) => {
        countrie.array = array;
    })
  };

  const arrayNewObject = { name: 'United States',
  currencyCode: 'USD',
  currencyName: 'United States Dollar',
};

  newObjetos(arrayNewObject);


  const findCountry = (countries, nome) => {
    const country = countries.find((countrie) => countrie.name === nome);
    if(country) {
  ...
  console.log(buildString(countries, 'Brazil','Brasilia - DF'));
